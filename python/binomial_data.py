...
def get_binomial_data(loc, sheet_name, upper_pd_inbound, upper_pd_outbound, no_rating):   
    ...
    master_scale_df = master_scale(upper_pd_inbound,upper_pd_outbound,no_rating)

    binning = master_scale_df['Average'].append(pd.Series([1]))
    test_set['Rating'] = pd.cut(test_set.PD, bins = binning, labels = master_scale_df['Rating'])
    ...
    binomial_table['no_default'] = test_set['Default'].groupby(test_set['Rating']).sum()
    binomial_table['PD'] = list(master_scale_df['Average'])
    binomial_table['lower_amount'] = binom.ppf(0.025,binomial_table['no_loan'], binomial_table['PD'])
    binomial_table['upper_amount'] = binom.ppf(0.975,binomial_table['no_loan'], binomial_table['PD'])
    binomial_table['difference'] = binomial_table['upper_amount'] - binomial_table['lower_amount']

    return [list(binomial_table['lower_amount']), list(binomial_table['difference'])], list(master_scale_df['Rating'])
...
if __name__ == '__main__':
    ...
    json_file = get_binomial_data(loc, sheet_name, upper_pd_inbound, upper_pd_outbound, no_rating)[0]
    filePathName = 'D:/Epay/Epay/Dashboard/dashboard_prototype/data/' + 'binomial_data' + '.json' 

    with open(filePathName, 'w') as fp:
        json.dump(json_file, fp)

    json_column_file = get_binomial_data(loc, sheet_name, upper_pd_inbound, upper_pd_outbound, no_rating)[1]
    fileName = 'D:/Epay/Epay/Dashboard/dashboard_prototype/data/' + 'binomial_column_data' + '.json' 
